...
def find_length_of_sequence(code, level):
    ...
    for char in code:
        ...
        all_moves = find_valid_seqs(start_pos, pos, pad["_"])
        if level > 0:
            length += min(find_length_of_sequence(s, level - 1) for s in all_moves)
        else:
            # in the last level the moves are all equal in length
            length += len(next(iter(all_moves)))
...
for code in codes:
    ...
    length = find_length_of_sequence(code, 2)
    print("CODE: ", code)
    print("LEN: ", length)
